...
{
    public partial class _CityChangeForm : Civ2form
    {
        ...
        public _CityChangeForm(City thisCity)
        {
            InitializeComponent();
        ...
        private void ChoicePanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            for (int row = 0; row < 16; row++)
            {
                ...
                if (BarValue + row < totalNoUnits)   //draw units
                {
                    ...
                    {
                        e.Graphics.DrawString(Game.Rules.UnitName[BarValue + row], new Font("Times New Roman", 16, FontStyle.Bold), new SolidBrush(Color.Black), new Point(85 + 1, row * 23 + 1));
                        e.Graphics.DrawString("(20 Turns, ADM: " + Game.Rules.UnitAttack[BarValue + row] + "/" + Game.Rules.UnitDefense[BarValue + row] + "/" + Game.Rules.UnitMove[BarValue + row] + " HP: " + Game.Rules.UnitHitp[BarValue + row] + "/" + Game.Rules.UnitFirepwr[BarValue + row] + ")", new Font("Times New Roman", 16, FontStyle.Bold), new SolidBrush(Color.Black), new Point(ChoicePanel.Width - VerticalBar.Width, row * 23 + 1), sf);
                    }
                    e.Graphics.DrawImage(ModifyImage.ResizeImage(Images.Units[BarValue + row], -4), new Point(1 + ((BarValue + row) % 2) * 38, 3 + row * 23 - 8));  //0.75-times the normal size of units
                    e.Graphics.DrawString(Game.Rules.UnitName[BarValue + row], new Font("Times New Roman", 16, fontstyle), new SolidBrush(textColor), new Point(85, row * 23));
                ...
                else    //draw improvements
                {
                    ...
                    if (BarValue + row == ThisCity.ItemInProduction)   //draw shadow of text for chosen line
                    {
                        e.Graphics.DrawString(Game.Rules.ImprovementName[improvNo], new Font("Times New Roman", 16, FontStyle.Bold), new SolidBrush(Color.Black), new Point(85 + 1, row * 23 + 1));
