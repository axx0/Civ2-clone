...
{
    public partial class _CityStatusForm : Civ2form
    {
        ...
        private void MainPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            foreach (City city in Game.GetCities.Where(n => n.Owner.Id == 1))
            {
                ...
                {
                    e.Graphics.DrawString(Game.Rules.ImprovementName[item - 62 + 1] + " (" + city.ShieldsProgress.ToString() + "/" + (10 * Game.Rules.ImprovementCost[item]).ToString() + ")", new Font("Times New Roman", 11, FontStyle.Bold), new SolidBrush(Color.FromArgb(67, 67, 67)), new Point(367 + 1, 82 + 24 * count + 1));
                    e.Graphics.DrawString(Game.Rules.ImprovementName[item - 62 + 1] + " (" + city.ShieldsProgress.ToString() + "/" + (10 * Game.Rules.ImprovementCost[item]).ToString() + ")", new Font("Times New Roman", 11, FontStyle.Bold), new SolidBrush(Color.FromArgb(223, 223, 223)), new Point(367, 82 + 24 * count));
                }               
                count++;
            }
            sf.Dispose();
