...
{
    public partial class _TradeAdvisorForm : Civ2form
    {
        ...
        private void MainPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            foreach (City city in Game.GetCities.Where(n => n.Owner.Id == 1))
            {
                ...
                for (int i = 0; i < 67; i++)
                {
                    if ((NoOfImprovements[i] > 0) && (Game.Rules.ImprovementUpkeep[i] > 0))  //only show improvements with upkeep > 0
                    {
                        e.Graphics.DrawString(NoOfImprovements[i].ToString() + " " + Game.Rules.ImprovementName[i] + " (Cost: " + UpkeepOfImprovements[i] + ")", new Font("Times New Roman", 11, FontStyle.Bold), new SolidBrush(Color.Black), new Point(335 + 1, 105 + 24 * count2 + 1));
                        e.Graphics.DrawString(NoOfImprovements[i].ToString() + " " + Game.Rules.ImprovementName[i] + " (Cost: " + UpkeepOfImprovements[i] + ")", new Font("Times New Roman", 11, FontStyle.Bold), new SolidBrush(Color.FromArgb(255, 223, 79)), new Point(335, 105 + 24 * count2));
                        count2++;
                    }
                }
