...
{
    public partial class _ScienceAdvisorForm : Civ2form
    {
        ...
        public _ScienceAdvisorForm()
        {
            ...
            HorizontalBar = new HScrollBar()
            {
                ...
                LargeChange = 1,
                Maximum = Game.GetCivs[1].Techs.Sum(x => x ? 1 : 0) / 8  //8 techs shown per column. Maximum=0 if no of techs <= 8, maximum=1 for techs=9...16, etc. (slider cannot move if maximum=0, it can move 1 move if maximum=1, it can move 2 moves if maximum=2, ...)
            };
            ...
            {
                if (Game.GetCivs[1].Techs[i]) DiscoveredTechs.Add(i);
            }
        }
        ...
        private void MainPanel_Paint(object sender, PaintEventArgs e)
        {
            ...
            e.Graphics.DrawString("Researching: " + Game.Rules.AdvanceName[Game.GetCivs[1].ReseachingTech], new Font("Times New Roman", 14), new SolidBrush(Color.FromArgb(67, 67, 67)), new Point(302 + 2, 74 + 1), sf);
            e.Graphics.DrawString("Researching: " + Game.Rules.AdvanceName[Game.GetCivs[1].ReseachingTech], new Font("Times New Roman", 14), new SolidBrush(Color.FromArgb(223, 223, 223)), new Point(302, 74), sf);
            ...
            for (int i = BarValue * 8; i < DiscoveredTechs.Count(); i++)
            {
                int x = 198 * (count / 8);
                int y = 22 * (count % 8);
                e.Graphics.DrawImage(Images.ResearchIcons[Game.Rules.AdvanceCategory[DiscoveredTechs[i]], Game.Rules.AdvanceEpoch[DiscoveredTechs[i]]], new Point(4 + x, 159 + y));
                e.Graphics.DrawString(Game.Rules.AdvanceName[DiscoveredTechs[i]], new Font("Times New Roman", 11, FontStyle.Bold), new SolidBrush(Color.FromArgb(67, 67, 67)), new Point(x + 42 + 2, y + 160 + 1));
                e.Graphics.DrawString(Game.Rules.AdvanceName[DiscoveredTechs[i]], new Font("Times New Roman", 11, FontStyle.Bold), new SolidBrush(Color.FromArgb(63, 187, 199)), new Point(x + 42, y + 160));
                count++;
